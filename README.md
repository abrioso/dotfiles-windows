...
-   `update.ps1`: Safely fast-forwards the current dotfiles branch and optionally replaces selected local JSON files from the latest `*.json.example` templates. Existing local files are backed up first; it never runs setup modules or applies configuration. When the Windows Terminal template is refreshed, offers to delete the generated `settings.json` so the next setup regenerates it (`-RegenerateTerminalSettings` for unattended runs).
