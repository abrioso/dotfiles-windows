...
-   `Set-UserHomeAlias.ps1`: Creates an ASCII-only junction alias for the user profile directory (for Entra ID displayName paths with accents) and points the user-scope `HOME` variable at it. It prefers the signed-in UPN reported by the in-box `whoami.exe /upn`, with validated identity/`USERNAME` fallbacks; no-op when the profile path is already ASCII-only.
